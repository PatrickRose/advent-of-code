...
class RegisterInstructions extends AbstractDay
{
...
INPUT;
    ...
    public function getAnswer(bool $partTwo): string
    {
        ...
        foreach(explode("\n", $input) as $line)
        {
            preg_match('/(\w+) (inc|dec) (-?\d+) if (\w+) (.+) (.+)/', $line, $matches);
            ...
            if (!isset($registers[$registerName]))
            {
                $registers[$registerName] = 0;
            }

            if (!isset($registers[$toCheck]))
            {
                $registers[$toCheck] = 0;
            }
            ...
            {
                case '>':
                    $toDo = $registers[$toCheck] > $comparison;
                    break;
                case '>=':
                    $toDo = $registers[$toCheck] >= $comparison;
                    break;
            ...
            if ($toDo)
            {
                $registers[$registerName] += $toChange;
            }

            if (max($registers) > $highestMax)
            {
                $highestMax = max($registers);
            }
        }
