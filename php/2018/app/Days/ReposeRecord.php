...
class ReposeRecord extends AbstractDay
{
...
INPUT;
    ...
    public function getAnswer(bool $partTwo): string
    {
        ...
        foreach ($rows as $row) {
            $matches = null;
            preg_match('/\[(\d{4}-\d{2}-\d{2}) (00|23):(\d{2})\] (falls asleep|wakes up|Guard \#(\d+) begins shift)/', $row, $matches);

            $day = new \DateTime($matches[1]);
            if ($matches[2] == '23') {
                $day->add(new \DateInterval('P1D'));
            }
            $day = $day->format('Y-m-d');
            $minutes = (int)($matches[2] == 23 ? '00' : $matches[3]);
            $event = $matches[4];
            ...
            switch ($event) {
                case 'wakes up':
                    $days[$day]['wakes'][] = $minutes;
                    break;
                case 'falls asleep':
                    $days[$day]['asleep'][] = $minutes;
                    break;
                default:
                    $days[$day]['id'] = $matches[5];
            }
        }

        // Now go through each day and sort out minute to minute
        // what's happening
        $awakes = [];
        $totalSleep = [];
        ...
        foreach ($days as $date => $day) {
            ...
            foreach (range(0, 59) as $minutes) {
                if (!$asleep) {
                    // Check if they fell asleep
                    if (array_search($minutes, $day['asleep']) !== false) {
                        $asleep = true;
                    }
                } else {
                    // Check if they woke up
                    if (array_search($minutes, $day['wakes']) !== false) {
                        $asleep = false;
                    }
                }

                $awakes[$id][$minutes] = ($awakes[$id][$minutes] ?? 0) + (int)$asleep;
                if (!isset($totalSleep[$id])) {
                    $totalSleep[$id] = 0;
                }
                if ($asleep) {
                    $totalSleep[$id] += 1;
        ...
        if ($partTwo) {
            ...
            $max = -1;
            $maxMinute = null;
            foreach ($awakes as $id => $minutes) {
                $thisMax = max($minutes);
                if ($thisMax > $max)
                {
                    $maxId = $id;
                    $max = $thisMax;
                    $maxMinute = array_search($thisMax, $minutes);
                }
            }
        } else {
            $maxId = array_search(max($totalSleep), $totalSleep);
            $maxMinute = array_search(max($awakes[$maxId]), $awakes[$maxId]);
        }
