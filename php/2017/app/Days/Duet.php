 ...
 * 0, and jgz because a is not greater than
 * 0). Finally, a is set to 1, causing the next jgz instruction to activate,
 ...
 * value?
 */
class Duet extends AbstractDay
{
...
INPUT;
    ...
    public function getAnswer(bool $partTwo): string
    {
        ...
        /** @var SendReceive[] $machines */
        $machines = [
            new SendReceive(0, $instructions, $partTwo),
            new SendReceive(1, $instructions, $partTwo)
        ];

        if (!$partTwo) {
            return $machines[0]->run();
        }
        ...
        $count = 0;
        while (true) {
            $toSend = $machines[$run]->run($toSend);

            $run = (int)(!$run);

            if (!is_array($toSend)) {
                $toSend = $machines[$run]->run();

                if ($run == 1) {
                    $count += is_array($toSend) ? count($toSend) : 0;
                }
            } else {
                if ($run == 0) {
                    $count += count($toSend);
                ...
                if (count($toSend) == 0 && $machines[$run]->waiting) {
                    return $count;
                }
            }
        }
