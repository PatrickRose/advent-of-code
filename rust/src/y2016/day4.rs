...
const PUZZLE_INPUT: &'static str = "aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
...
pub fn calculate(part2: bool) {
    if part2 {
        for line in PUZZLE_INPUT.lines() {
            if valid_room_number(line) {
                println!("{} is in {}", unencrypt_name(line), get_room_id(line));
            }
        }
    } else {
        let mut count = 0;
...
pub fn unencrypt_name(input: &str) -> String {
    let mut output = String::with_capacity(input.len());

    let shift = get_room_id(input) % 26;
    ...
    for c in input.chars() {
        ...
        for _ in 0..shift {
            match answer {
                ...
                '-' => { answer = ' ' },
                ' ' => { answer = ' ' },
                _ => { return output.trim().to_string() }
            }
        }
        match answer {
            'a' => { output.push('a') },
...
pub fn valid_room_number(input: &str) -> bool {
    lazy_static! {
        static ref RE: Regex = Regex::new(r"^(.+)-\d+\[(\w{5})\]$").unwrap();
    }
    lazy_static! {
        static ref CHARACTERS: Regex = Regex::new(r"^(\d+)(\w)$").unwrap();
    }
    ...
    for cap in RE.captures_iter(input) {
        ...
        let mut sort_vec = Vec::with_capacity(counts.len());

        for (character, count) in &counts {
            sort_vec.push(format!("{}{}", count, character));
        }
...
pub fn get_room_id(input: &str) -> usize {
    lazy_static! {
        static ref RE: Regex = Regex::new(r"^.+?(\d+)\[\w{5}\]$").unwrap();
    }
...
mod tests {
    ...
    #[test]
    pub fn we_can_get_the_room_id() {
        let input = "aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]";

        assert_eq!(561, get_room_id(input), "{} is 561", input);
    }

    #[test]
    pub fn we_can_test_if_a_room_is_valid() {
        const INPUT: &'static str = "aa-bbb-z-y-x-123[abxyz]
a-b-c-d-e-f-g-h-987[abcde]
not-a-real-room-404[oarel]";
        for line in INPUT.lines() {
            assert!(valid_room_number(&line), "{} is a valid room number", line);
        }

        assert!(!valid_room_number("totally-real-room-200[decoy]"), "totally-real-room-200[decoy] is not a valid room number");
    }

    #[test]
    pub fn we_can_do_a_shift_cipher() {
        let input = "qzmt-zixmtkozy-ivhz-343[abcde]";

        assert_eq!("very encrypted name", unencrypt_name(input));
    }
