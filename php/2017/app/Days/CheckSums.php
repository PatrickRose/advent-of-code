...
class CheckSums extends AbstractDay
{
...
;
    ...
    {
        $sum = 0;
        foreach(explode(PHP_EOL, $this->getPuzzleInput()) as $line)
        {
            $values = preg_split('/\s/', $line);

            if ($partTwo)
            {
                foreach($values as $i => $firstVal)
                {
                    foreach($values as $j => $secondVal)
                    {
                        if ($i == $j)
                        {
                            continue;
                        ...
                        $max = max($firstVal, $secondVal);
                        $min = min($firstVal, $secondVal);
                        if ($max % $min == 0)
                        {
                            $sum += ($max / $min);
                            break 2;
