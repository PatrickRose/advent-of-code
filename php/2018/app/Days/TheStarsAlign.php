...
class TheStarsAlign extends AbstractDay
{
...
INPUT;
    ...
    public function getAnswer(bool $partTwo): string
    {
        ...
        $progress->setFormat("%current% - %message%");
        $progress->start();
        ...
        foreach (explode("\n", $input) as $item) {
            ...
            $grid[$matches[1]][$matches[2]] = [$matches[3], $matches[4]];
        }
        ...
        do {
            ...
            foreach ($grid as $x => $row) {
                foreach ($row as $y => $cell) {
                    // As a Y increases, it goes further down
                    $newY = $y + $cell[1];
                    $newGrid[$x + $cell[0]][$newY] = $cell;
            ...
            $minX = min(array_keys($newGrid));
            $maxX = max(array_keys($newGrid));

            // Check to see if we've finished now
            $newSize = ($maxX - $minX) * ($maxY - $minY);

            if ($newSize > $previousSize) {
                $progress->setMessage('Found!');
                $progress->finish();
                $this->output->writeln('');
                ...
                // Stop! It's increasing, it'll never look as good
                $answer = '';
                for ($y = $minY; $y <= $maxY; $y++) {
                    for ($x = $minX; $x <= $maxX; $x++) {
                        if (isset($grid[$x][$y])) {
            ...
            $progress->setMessage('Size is now ' . $previousSize);
            $progress->advance();
        } while (true);
    }
}
