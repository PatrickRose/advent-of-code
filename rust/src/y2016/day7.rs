const PUZZLE_INPUT: &'static str = "dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
vvvzhcfuecgfvrxrnquo[oqgutuxthxlcxhpke]liqjotlxzbmsassyxrf[colshvmiwbfjansdg]vggdkkyqrjvthtvp[dmozaqtceghrabasafj]lnsoewepnlbqvibyk
...
{
    println!("{}", get_count_of_valid_ips(PUZZLE_INPUT, part2));    
}

pub fn get_count_of_valid_ips(input: &str, support_ssl: bool) -> usize {
    let mut count = 0;

    for line in input.lines() {
        if support_ssl && supports_ssl(line) {
            count += 1;
        }
        if !support_ssl && supports_tls(line) {
            count += 1;
        }
    }
...
pub fn supports_tls(input: &str) -> bool {
    let mut in_brackets = false;
    let mut first_char = ' ';
    let mut second_char = ' ';
    let mut position_in_string = 0;
...
pub fn supports_ssl(input: &str) -> bool {
    let mut hypernets = String::new();
    let mut supernets = String::new();
    ...
    for char in input.chars() {
        ...
        match in_hypernet {
            true => hypernets.push(char),
            false => supernets.push(char),
        }
    }

    for aba in calculate_aba_list(&supernets) {
...
pub fn calculate_aba_list(input: &str) -> Vec<String> {
    ...
    for char in input.chars() {
        ...
        if char == first_char {
            to_return.push(format!("{}{}{}", second_char, first_char, second_char));
        }
...
mod tests {
    ...
    #[test]
    pub fn can_validate_tls() {
        assert!(supports_tls("abba[mnop]qrst"), "abba[mnop]qrst should be a valid ip");
        assert!(supports_tls("ioxxoj[asdfgh]zxcvbn"), "ioxxoj[asdfgh]zxcvbn should be a valid ip");
        assert!(!supports_tls("abcd[bddb]xyyx"), "abcd[bddb]xyyx should not be a valid ip");
        assert!(!supports_tls("aaaa[qwer]tyui"), "aaaa[qwer]tyui should not be a valid ip");
    }

    #[test]
    pub fn can_count_ips_support_tls() {
        assert_eq!(2, get_count_of_valid_ips("abba[mnop]qrst
...
aaaa[qwer]tyui", false));
    ...
    #[test]
    pub fn can_validate_ssl() {
        assert!(supports_ssl("aba[bab]xyz"), "aba[bab]xyz should be a valid ip");
        assert!(!supports_ssl("xyx[xyx]xyx"), "xyx[xyx]xyx should not be a valid ip");
        assert!(supports_ssl("aaa[kek]eke"), "aaa[kek]eke should be a valid ip");
        assert!(supports_ssl("zazbz[bzb]cdb"), "zazbz[bzb]cdb should not be a valid ip");
        assert!(!supports_ssl("nqwjdcnwfxkdglllft[gbawkxvzhyiprfenf]ysybkzwywpqwerm[cwsthmeytiuialllzxx]plcctxffnigyhdfmndc[kyyvjcfkxfofxfsrw]cwynasabqneione"), "nqwjdcnwfxkdglllft[gbawkxvzhyiprfenf]ysybkzwywpqwerm[cwsthmeytiuialllzxx]plcctxffnigyhdfmndc[kyyvjcfkxfofxfsrw]cwynasabqneione should not be a valid ip");
    }

    #[test]
    pub fn can_count_ips_supporting_ssl() {
        assert_eq!(3, get_count_of_valid_ips("aba[bab]xyz
xyx[xyx]xyx
aaa[kek]eke
zazbz[bzb]cdb", true));
    }
