...
pub fn calculate_word(input: &str) -> String {
    ...
    while to_parse.len() > 0 {
        match to_parse.find('(') {
            Some(index) => {
                to_return = to_return + &to_parse[..index];
                to_parse = &to_parse[(index+1)..];
                ...
                match to_parse.find(')') {
                    Some(closing) => {
                        ...
                        for cap in MARKER.captures_iter(&to_parse[..closing]) {
                            ...
                            to_parse = &to_parse[(closing+1+chars_to_read)..];
                        }
                    },
                    None => {
