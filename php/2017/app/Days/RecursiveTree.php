...
class RecursiveTree extends AbstractDay
{
...
INPUT;
    ...
    public function getAnswer(bool $partTwo): string
    {
        ...
        if ($partTwo) {
            return $this->findUnbalancedNode(current($tree));
        } else {
            return key($tree);
        }
    }
    ...
    private function buildTree()
    {
        ...
        // Convert the input into the format we need
        $tree = [];
        ...
        foreach ($input as $line) {
            ...
            $tree[$key] = compact('weight', 'subs');
        }

        do {
            $unset = [];
            foreach ($tree as $key => $item) {
                if (in_array($key, $unset) || is_array($item['subs'])) {
                    // Already folded
                    continue;
                }

                if (is_null($item['subs'])) {
                    // Can't fold yet
                    continue;
                }

                // So subs must be a string
                $canUnset = true;
                $newSubs = [];
                foreach (explode(', ', $item['subs']) as $sub) {
                    if (is_string($tree[$sub]['subs'])) {
                        // Can't do this yet, have to wait until a new iteration
                        $canUnset = false;
                        break;
                    }

                    $newSubs[$sub] = $tree[$sub];
                }

                if ($canUnset) {
                    $tree[$key]['subs'] = $newSubs;
                    $subWeight = 0;
                    foreach ($newSubs as $toUnset => $sub) {
                        $unset[] = $toUnset;
                        $subWeight += $sub['weight'] + ($sub['subweight'] ?? 0);
                        unset($tree[$toUnset]);
                    }
                    $tree[$key]['subweight'] = $subWeight;
                }
            }
        } while (count($tree) !== 1);
    ...
    private function findUnbalancedNode(array $tree)
    {
        $weights = [];
        foreach ($tree['subs'] as $key => $sub)
        {
            $weights[$key] = $sub['weight'] + ($sub['subweight'] ?? 0);
        }
        ...
        $minCount = array_search(min($counts), $counts);
        $unbalanced = array_search($minCount, $weights);

        $node = $this->findUnbalancedNode($tree['subs'][$unbalanced]);
        ...
        if ($node === false)
        {
            ...
            $subweight = $tree['subs'][$unbalanced]['subweight'];
            if ($tree['subs'][$unbalanced]['weight'] + $subweight == max($counts))
            {
                return min($counts) - $subweight;
            }
            else
            {
                return max($counts) - $subweight;
            }
        }
        else
        {
