 ...
 * buffer some number of steps (your puzzle input) before inserting the first
 * new value, 1, after the value it stopped on. The inserted value becomes the
 * current position. Then, it steps forward from there the same number of steps,
 * and wherever it stops, inserts after it the second new value, 2, and uses that
 ...
 */
class Spinlock extends AbstractDay
{
    ...
    {
        $steps = $this->getPuzzleInput();

        $buffers = [0];
        $pos = 0;

        $partTwoAnswer = null;
        $iterations = $partTwo ? 50000000 : 2018;
        ...
        for ($i = 1; $i < $iterations; $i++) {
            ...
            if ($partTwo) {
                ...
                $before = array_slice($buffers, 0, $pos + 1);
                $after = count($buffers) == ($pos + 1) ? [] : array_slice($buffers, $pos + 1);

                $buffers = array_merge($before, [$i], $after);
            }
            $pos++;
        }

        return $partTwo ? $partTwoAnswer : $buffers[$pos + 1];
    }
}
